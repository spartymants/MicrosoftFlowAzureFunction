...
namespace FlowConnectorService
{
    public static class Function1
    {
        ...
        public static IActionResult Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = null)]HttpRequest req, TraceWriter log)
        {
            ...
            string strProcess = req.Query["process"];
            string strResource = req.Query["resource"];
            string strPort = null;

            if (strResource.Contains(":")){
                int charPos = strResource.IndexOf(":");
                strPort = strResource.Substring(charPos+1);
                strResource = strResource.Substring(0, charPos);
            } else { strPort = "8181"; }
            ...
            HttpWebRequest request = CreateSOAPWebRequest(strProcess, strResource, strPort);
            ...
            var soapString = ConstructSoapRequest(strProcess, convertXML, log);
            ...
            var responseNode = SOAPResponseBody.GetElementsByTagName(strProcess + "Response")[0];
        ...
        private static string ConstructSoapRequest(String processName, String xmlPayload, TraceWriter log)
        {
